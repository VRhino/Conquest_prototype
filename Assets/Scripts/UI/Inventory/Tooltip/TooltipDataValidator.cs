...
public class TooltipDataValidator : ITooltipComponent
{
    ...
    /// <summary>
    /// Valida que los parámetros básicos del ítem sean correctos.
    /// </summary>
    public bool ValidateItemParameters(InventoryItem item, ItemData itemData)
    {
        if (item == null || itemData == null) return false;
        return InventoryUtils.ValidateItemParameters(item.itemId);
    }

    /// <summary>
    /// Verifica si un ítem es válido para mostrar en tooltip.
    /// </summary>
    public bool IsItemValidForTooltip(InventoryItem item)
    {
        if (item == null) return false;

        var currentHero = PlayerSessionService.SelectedHero;
        if (currentHero?.inventory == null) return false;

        return InventoryUtils.IsTooltipItemValid(item, currentHero.inventory);
    }

    /// <summary>
    /// Valida que los componentes UI necesarios estén presentes.
    /// </summary>
    public bool ValidateUIComponents()
    {
        if (_controller.TooltipPanel == null)
        {
            Debug.LogWarning("[TooltipDataValidator] tooltipPanel no asignado");
            return false;
        }

        if (_controller.TitlePanel == null)
        {
            Debug.LogWarning("[TooltipDataValidator] titlePanel no asignado");
            return false;
        }

        if (_controller.ContentPanel == null)
        {
            Debug.LogWarning("[TooltipDataValidator] contentPanel no asignado");
            return false;
        }

        if (_controller.InteractionPanel == null)
        {
            Debug.LogWarning("[TooltipDataValidator] interactionPanel no asignado");
            return false;
        }
