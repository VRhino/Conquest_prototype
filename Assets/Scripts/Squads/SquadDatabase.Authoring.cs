    ...
    class Baker : Unity.Entities.Baker<SquadDatabaseAuthoring>
    {
        public override void Bake(SquadDatabaseAuthoring authoring)
        {
            ...
            foreach (var squadData in authoring.database.allSquads)
            {
                ...
                {
                    vidaBase = squadData.baseHealth,
                    velocidadBase = squadData.baseSpeed,
                    masa = squadData.massValue,
                    peso = squadData.totalWeight,
                    squadType = squadData.type,
                    bloqueo = squadData.block,
                    defensaCortante = squadData.slashingDefense,
                    defensaPerforante = squadData.piercingDefense,
                    defensaContundente = squadData.bluntDefense,
                    danoCortante = squadData.slashingDamage,
                    danoPerforante = squadData.piercingDamage,
                    danoContundente = squadData.bluntDamage,
                    penetracionCortante = squadData.slashingPenetration,
                    penetracionPerforante = squadData.piercingPenetration,
                    penetracionContundente = squadData.bluntPenetration,
                    esUnidadADistancia = squadData.isDistanceUnit,
                    alcance = squadData.range,
                    precision = squadData.accuracy,
                    cadenciaFuego = squadData.fireRate,
                    velocidadRecarga = squadData.reloadSpeed,
                    municionTotal = squadData.ammo,
                    liderazgoCost = squadData.leadershipCost,
